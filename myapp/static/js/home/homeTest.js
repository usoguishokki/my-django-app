...
// DOMContentLoaded後に高さを確認する
document.addEventListener('DOMContentLoaded', () => {
    debugGanttHeight();
    analyzeGanttScrollHeightAccurate(); // ← ここを追加
    checkGanttPadding()
});

function debugGanttHeight() {
    const gantt = document.getElementById('gantt');
    if (!gantt) {
        console.warn('#gantt not found');
        return;
    }

    const computedStyle = window.getComputedStyle(gantt);
    const childElements = Array.from(gantt.children);

    console.log('--- #gantt Debug Info ---');
    console.log('Element:', gantt);
    console.log('Computed height:', computedStyle.height);
    console.log('Scroll height:', gantt.scrollHeight);
    console.log('Client height:', gantt.clientHeight);
    console.log('Offset height:', gantt.offsetHeight);
    console.log('Overflow-y:', computedStyle.overflowY);
    console.log('Child elements count:', childElements.length);

    childElements.forEach((child, index) => {
        const rect = child.getBoundingClientRect();
        console.log(`Child[${index}] tag:${child.tagName} height:${rect.height}px`);
    });

    console.log('--- END ---');
}

function analyzeGanttScrollHeightAccurate() {
    const gantt = document.getElementById('gantt');
    if (!gantt) {
        console.warn('#gantt not found');
        return;
    ...
    const children = Array.from(gantt.children);

    console.log('--- Accurate Scroll Height Analysis ---');
    let maxBottom = ganttRect.top;

    children.forEach((child, index) => {
        const rect = child.getBoundingClientRect();
        console.log(
            `Child[${index}] tag:<${child.tagName}> rect.top=${rect.top}px, rect.bottom=${rect.bottom}px, height=${rect.height}px`
        );
    ...
    const contentHeight = maxBottom - ganttRect.top;

    console.log(`=> Calculated content height (rect): ${contentHeight}px`);
    console.log(`=> Actual scrollHeight: ${gantt.scrollHeight}px`);
    console.log('--- END ---');
}

function checkGanttPadding() {
    const gantt = document.getElementById('gantt');
    if (!gantt) {
        console.warn('#gantt not found');
        return;
    ...
    console.log('--- #gantt Padding Info ---');
    console.log('padding-top:', computedStyle.paddingTop);
    console.log('padding-bottom:', computedStyle.paddingBottom);
    console.log('border-top-width:', computedStyle.borderTopWidth);
    console.log('border-bottom-width:', computedStyle.borderBottomWidth);
    console.log('--- END ---');
}